...
const updateProfile = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { name, name_kana, birthdate, sex, zip, prefectures, tel, desired_working_place_1, desired_working_place_2, other_desired_criteria, employment_types, desired_conditions, } = req.body;
        const jobSeeker = await JobSeeker.findByPk(id);
        if (!jobSeeker || jobSeeker.deleted) {
            throw new NotFoundError("Job seeker not found");
        }
 ...
 */
const getFavoriteJobs = async (req, res, next) => {
    try {
        const { id } = req.user;
 ...
 */
const addFavoriteJob = async (req, res, next) => {
    try {
 ...
 */
const removeFavoriteJob = async (req, res, next) => {
    try {
        const { id: jobSeekerId } = req.user;
    ...
    try {
        ...
        if (searchTerm) {
            ...
            whereCondition[sequelize_1.Op.or] = [
                ...
                { name_kana: { [sequelize_1.Op.like]: `%${searchTerm}%` } },
...
const deleteJobSeekerById = async (req, res, next) => {
    try {
        const { id } = req.params;
        const jobSeeker = await JobSeeker.findByPk(id);
        if (!jobSeeker) {
